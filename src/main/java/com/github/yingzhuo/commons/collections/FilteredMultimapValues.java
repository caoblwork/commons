...
final class FilteredMultimapValues<K, V> extends AbstractCollection<V> {
  ...
  public boolean removeAll(Collection<?> c) {
    return Iterables.removeIf(multimap.unfiltered().entries(),
        ...
        PredicateUtils.<Entry<K, V>>and(multimap.entryPredicate(),
            Maps.<V>valuePredicateOnEntries(PredicateUtils.in(c))));
  }
  ...
  public boolean retainAll(Collection<?> c) {
    return Iterables.removeIf(multimap.unfiltered().entries(),
        // explicit <Entry<K, V>> is required to build with JDK6
        PredicateUtils.<Entry<K, V>>and(multimap.entryPredicate(),
            Maps.<V>valuePredicateOnEntries(PredicateUtils.not(PredicateUtils.in(c)))));
  }

...
public class ChainedTransformer <I,O> implements Transformer<I, O>, Serializable {
	...
	public static <I,O> Transformer<I, O> getInstance(Transformer<I,O>[] transformers) {
        FunctorUtils.validate(transformers);
        if (transformers.length == 0) {
            return (Transformer<I, O>) NOPTransformer.getInstance();
        }
        transformers = (Transformer<I, O>[]) FunctorUtils.copy(transformers);
	...
	public static <I,O> Transformer<I, O> getInstance(Collection<Transformer<I, O>> transformers) {
        ...
        int i = 0;
        for (Iterator<Transformer<I, O>> it = transformers.iterator(); it.hasNext();) {
            cmds[i++] = it.next();
        }
     ...
     */
    public ChainedTransformer(Transformer<I, O>[] transformers) {
        super();
     ...
     */
    @SuppressWarnings("unchecked")
	public Transformer<I, O>[] getTransformers() {
